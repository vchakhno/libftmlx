...
/*   Updated: 2023/06/23 06:19:06 by vchakhno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_solid_point(
	t_renderer *renderer, t_u32 x, t_u32 y, t_color color
) {
	if (x < renderer->width && y < renderer->height)
		*ft_image_get_pixel(&renderer->back_buffer, x, y) = color;
}

void	ft_draw_solid_point__unchecked(
	t_renderer *renderer, t_u32 x, t_u32 y, t_color color
) {
	*ft_image_get_pixel(&renderer->back_buffer, x, y) = color;
}

void	ft_draw_opaque_point(
	t_renderer *renderer, t_point point, t_color color, float opacity
) {
	...
	t_point point, t_color color, float opacity)
{
	...
	old_pixel = ft_image_get_pixel(&renderer->back_buffer, point.x, point.y);
