...
/*   Updated: 2023/06/23 08:49:40 by vchakhno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_image_copy_image(
	...
	ft_rect_overlap(
		...
		&total_normalized);
	ft_image_copy_image__unchecked(
...
void	ft_image_copy_image__unchecked(
	...
	while (dst_i < (t_u32) dst_rect.h)
	{
		...
		while (dst_j < (t_u32) dst_rect.w)
		{
			...
			dst_pixel = ft_image_get_pixel(dst,
					dst_j + (t_u32) dst_rect.x, dst_i + (t_u32) dst_rect.y);
			*dst_pixel = ft_color_blend(
					*dst_pixel, *ft_image_get_pixel(src, src_j, src_i));
			dst_j++;
		}
		dst_i++;
	}
